...
/* ✅ ✅ ✅ HARD-CODED MUNICIPAL + INDUSTRIAL TERMINOLOGY SYSTEM (AUTO FROM PREDICTION) */
const categoryTerminologyMap = {
  "Fresh Water": {
    title: "Municipal Drinking Water Treatment (WTP)",
    flow: [
      "Raw Water Intake (River / Groundwater)",
  ...
  "Irrigation Water": {
    title: "Municipal Wastewater Reuse (STP / WWTP)",
    flow: [
  ...
  "Cooling System": {
    title: "Industrial Water Treatment & Recycle (ETP + RO)",
    flow: [
  ...
  "Unusable": {
    title: "High-Contamination Industrial Effluent",
    flow: [
      "Heavy Metal Presence",
      "Toxic Chemical Load",
      "High Salinity Brine",
      "Non-Biodegradable Sludge"
    ],
    reuse: [
      "Thermal Evaporation Required",
...
export default function MLPredictionPanel({ batchData, onPredictionComplete }) {
  ...
  if (!batchData) return;
   ...
   setIsAnalyzing(true);
    ...
    const timer = setTimeout(async () => {
      ...
      setPredictions(formatted);
      setRecommendedUse(formatted[0].name);
  ...
  const terminology = categoryTerminologyMap[recommendedUse];
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        ...
        {/* ✅ ✅ ✅ TERMINOLOGY DISPLAY PANEL */}
        {terminology && !isAnalyzing && (
          <div className="mt-8 glassmorphism rounded-2xl p-6 border border-white/30">
            <h3 className="text-2xl font-bold mb-4 text-shakespeare-800">
              {terminology.title}
            </h3>

            <h4 className="font-semibold mb-2">🔄 Treatment Flow</h4>
            <ul className="list-disc ml-6 mb-4">
              {terminology.flow.map((step, i) => (
                <li key={i}>{step}</li>
              ))}
            </ul>

            <h4 className="font-semibold mb-2">♻️ Reuse Applications</h4>
            <ul className="list-disc ml-6">
              {terminology.reuse.map((step, i) => (
                <li key={i}>{step}</li>
              ))}
            </ul>
          </div>
