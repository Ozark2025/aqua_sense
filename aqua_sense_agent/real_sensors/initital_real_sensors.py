...
#     import uvicorn
#     uvicorn.run("arduino_stream_api:app", host="0.0.0.0", port=8001, reload=True)
...
def parse_line(line: str):
    ...
    # ✅ SAFETY FIX (prevents 'type is not iterable' crash)
...
def normalize_timestamp(data: Dict[str, Any]) -> None:
    ...
    dt = dt.astimezone(timezone.utc)
    data["timestamp"] = dt.isoformat(timespec="microseconds")
        ...
        while True:
            try:
                ...
                if data:
                    ...
                    # ✅ ADD RANDOM FLOW DATA (ANY NORMAL RANGE)
                    data["flow"] = round(random.uniform(20, 40), 2)
